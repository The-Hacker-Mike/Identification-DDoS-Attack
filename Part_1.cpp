...
/*
void mostrarPuertosDestino(vector<Record> v)
{
    vector<int> puertosNum;
    int i = 0, j = 0;
    while (i < v.size())
    {
        if (v[i].puertoDestino != "-")
        {
            puertosNum.push_back(stoi(v[i].puertoDestino));
            //v[i].puertoDestino = "0";
            //puertosNum.push_back(stoi(v[i].puertoDestino));
            i++;
        }
    ...
    i = 0;
    while (i < puertosNum.size())
    {
        if (puertosNum[i] < 1000)
        {
            cout << puertosNum[j] << endl;
            i++;
        }
        i++;
    }
...
int puertosAbajoDeMil(vector<Record> v)
{
    ...
    while (i < v.size())
    {
        puertos.push_back(v[i].puertoDestino);
        i++;
    }
...
/*
void mostrarNombresDestinoUnicos(vector<Record> a)
{
    vector<string> nombres;
    string palabraComparar = a[0].nombreDestino;
   ...
   {
       if (nombres[7].find(palabraComparar) != nombres[7].npos)
       {
